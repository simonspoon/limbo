...
// machinery runs. When the parent test re-execs the test binary with
// LIMBO_LOCK_HELPER=1, we call AcquireLock on the supplied directory and
// exit with the documented exit code (0 on success, 4 on contention, 1 on
// any other error).
...
func TestLockContention_CrossProcess(t *testing.T) {
	...
	// Parent acquires the lock first.
	unlock, err := AcquireLock(dir)
	require.NoError(t, err)
	defer unlock()

	// Re-exec this test binary, running only this same test's helper branch
	// via TestMain — the env flag short-circuits before Run.
	...
	// The child should exit with code 4 (lock contention).
	require.Error(t, err, "child should fail due to lock contention; stdout/stderr: %s", string(out))
...
func TestLockRelease_DoesNotRemoveFile(t *testing.T) {
	...
	// Guards against a future "cleanup" regression that would reintroduce
	// the inode race. The lock file MUST remain on disk after unlock.
